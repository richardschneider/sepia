...
icon="http://www.ascc.net/xml/resource/schematron/bilby.jpg">
  ...
  <p>
    Copyright (C) 2001 Rick Jellife.
    Freely and openly available under zlib/libpng license.
  </p>
  <p>
    This schema is open: it only
  ...
  <p>
    This schema uses conservative rules (e.g. no use of key()) to
    work with incomplete XSLT-based implementations.
  </p>
  ...
  <pattern name="Minimal Schematron" id="mini">
    <p>
      These rule establish the smallest possible Schematron document.
      These rules may be handy for beginners with starting documents.
    </p>
    <rule context="/">
  ...
  <pattern name="Schematron Elements and Required Attributes" id="required">
    ...
    <rule context="sch:schema">
      <assert test="count(sch:*) = count(sch:title|sch:ns|sch:phrase|sch:p|sch:pattern|sch:diagnostics|sch:phase)"
		>
        The element <name/> should contain only the elements title, ns, phrase, p, pattern, diagnostics or phase from the Schematron namespace.
      </assert>
      ...
      <report test="@defaultPhase and sch:phase and not(@defaultPhase='#ALL') and not(@defaultPhase='#DEFAULT') and not(sch:phase[@id= current()/@defaultPhase])"
    ...
    <rule context="sch:title">
      ...
      <assert test="count(preceding-sibling::sch:*) = 0"
		>
        The element <name/> should only appear as the first element from the Schematron namespace in the schema element.
      </assert>
    </rule>
    <rule context="sch:ns">
      <assert test="parent::sch:schema"  diagnostics="bad-parent"
      ...
      <assert test="string-length(normalize-space(@prefix)) &gt; 0"
		>
        The element <name/> must have a value for the attribute prefix.
      </assert>
      <assert test="string-length(normalize-space(@uri)) &gt; 0"
		>
        The element <name/> must have a value for the attribute uri.
      </assert>
      <assert test="count(preceding-sibling::sch:*) = count(preceding-sibling::sch:title) + count(preceding-sibling::sch:ns)"
		>
        The <name/> element must come before any other Schematron elements, except the title
      </assert>
      <report test="*"
		>
    ...
    <rule context="sch:phase">
      ...
      <assert test="count(preceding-sibling::sch:*) = count(preceding-sibling::sch:phase)
		...
		>
        The <name/> elements must come before any other Schematron elements, except the title, ns and p elements
      </assert>
    </rule>
    <rule context="sch:active">
      <assert test="parent::sch:phase"  diagnostics="bad-parent"
		>
        The element <name/> should only appear inside the Schematron element phase.
      </assert>
      <assert test="string-length(normalize-space(@pattern)) &gt; 0"
		>
        The element <name/> must have a value for the attribute pattern.
      </assert>
    </rule>
    <rule context="sch:pattern">
      ...
      <assert test="count(sch:*) = count(sch:rule|sch:p)"
		>
        The element <name/> should contain only rule and p elements from the Schematron namespace.
      </assert>
      <assert test="sch:rule"
		>
        The element <name/> should contain at least one rule element.
      </assert>
      <assert test="string-length(normalize-space(@name)) &gt; 0"
		>
        The element <name/> must have a value for the attribute name.
      </assert>
    ...
    <rule context="sch:rule[@abstract='true']">
      ...
      <assert test="count(sch:*) = count(sch:assert |sch:report|sch:key|sch:extends ) "
		>
        The element <name/> should contain only the elements assert, report, key or extends from the Schematron namespace.
      </assert>
      ...
      <report test="@test"
		>
        The <name/> element cannot have a test attribute: that should go on a report or assert element.
      </report>
      <report test="@context"
    ...
    <rule context="sch:assert ">
      ...
      <assert test="string-length(normalize-space(text())) &gt; 0"
		>
        A <name/> element should contain a natural language sentence.
      </assert>
      <assert test="string-length(normalize-space(@test)) &gt; 0"
		>
        The element <name/> must have a value for the attribute test. This should be an XSLT expression.
      </assert>
      <report test="@context"
		>
        The <name/> element cannot have a context attribute: that should go on the rule element.
      </report>
    </rule>
    ...
    <rule context="sch:diagnostics">
      <assert test="parent::sch:schema"  diagnostics="bad-parent"
		>
        The element <name/> should only appear as a child of the schema element
      </assert>
      <report test="following-sibling::sch:*"
		>
        The element <name/> should be the last element in the schema.
      </report>
    </rule>
    <rule context="sch:diagnostic">
      <assert test="parent::sch:diagnostics"  diagnostics="bad-parent"
		>
        The element <name/> should only appear in the diagnostics section.
      </assert>
      <assert test="string-length(normalize-space(@id)) &gt; 0"
		>
        The element <name/> must have a value for the attribute id.
      </assert>
    </rule>
    <rule context="sch:key">
      <assert test="parent::sch:rule"  diagnostics="bad-parent"
		>
        The element <name/> should only appear in a rule.
      </assert>
      <assert test="string-length(normalize-space(@name)) &gt; 0"
		>
        The element <name/> must have a value for the attribute name.
      ...
      <assert test="string-length(normalize-space(@path)) &gt; 0"
		>
        The element <name/> must have a value for the attribute path.   This should be an XPath expression.
      </assert>
      <report test="*"
		>
        The <name/> element should be empty.
    ...
    <rule context="sch:extends">
      ...
      <assert test="string-length(normalize-space(@rule)) &gt; 0"
		>
        The element <name/> must have a value for the attribute rule.
      </assert>
      <report test="*"
		>
        The <name/> element should be empty.
      ...
      <assert test="/*//sch:rule[@abstract='true'][@id = current()/@rule]"
		>
        The <name/> element should have an attribute rule which gives the id of an abstract rule.
      </assert>
    </rule>
    <rule context="sch:p">
      <assert test="parent::sch:*"  diagnostics="bad-parent"
		>
        The element <name/> should only appear inside an element from the Schematron namespace. It is equivalent to the HTML element of the same name.
      </assert>
    </rule>
    <rule context="sch:name">
      <assert test="parent::sch:assert | parent::sch:report |parent::sch:p | parent::sch:diagnostic"
       diagnostics="bad-parent"
		>
        The element <name/> should only appear inside a Schematron elements p (paragraph) or diagnostic.
      </assert>
      <report test="*"
		>
        The <name/> element should be empty.
    ...
    <rule context="sch:emph">
      <assert test="parent::sch:p | parent::sch:diagnostic"
       diagnostics="bad-parent"
		>
        The element <name/> should only appear inside a Schematron elements p (paragraph) or diagnostic. It is equivalent to the HTML element of the same name.
      </assert>
    </rule>
    <rule context="sch:dir">
      <!-- rps added title as a valid parent -->
      <assert test="parent::sch:p | parent::sch:diagnostic | parent::sch:title"
       diagnostics="bad-parent"
		>
        The element <name/> should only appear inside a Schematron elements p (paragraph) or diagnostic.
      </assert>
      <assert test="@value and (@value='rtl' or @value='ltr')"
		>
        The attribute value of the <name/> element must be lowercase "rtl" or "ltr". It is equivalent to the HTML element of the same name.
      </assert>
    </rule>
    ...
    <rule context="sch:value-of">
      <assert test="parent::sch:diagnostic"   diagnostics="bad-parent"
		>
        The element <name/> should only appear inside the Schematron element diagnostic.
      </assert>
      <assert test="string-length(normalize-space(@select)) &gt; 0"
		>
        The element <name/> must have a value for the attribute select. The value should be an XPath expression.
      </assert>
      <report test="*"
		>
        The <name/> element should be empty.
    ...
    <rule context="sch:*">
      <report test="1=1" diagnostics="spelling"
		>
        The <name/> element is not an element from the Schematron 1.5 namespace
      </report>
    </rule>
  </pattern>
  ...
  <pattern name="Schematron Attributes" id="attributes" >
    ...
    <rule context="sch:*">
      ...
      <report test="@icon and not(self::sch:schema or self::sch:report or
		...
		or self::sch:phase or self::sch:report or self::sch:assert)"
		>The attribute icon can only appear on the elements schema, report, diagnostic, key, p, pattern, phase and assert. It is the URL of a small image. </report>
      <report test="@id and not(self::sch:schema or self::sch:report or
		self::sch:p or self::sch:pattern or self::sch:phase or 
		self::sch:assert or self::sch:report or self::sch:rule or self::sch:diagnostic)"
		>The attribute id can only appear on the elements schema, assert, report, p, pattern, phase, report, rule and diagnostic. It is a name, it should not start with a number or symbol.</report>
      ...
      <report test="@role and not(self::sch:report or self::sch:assert or self::sch:rule)"
		>The attribute role can only appear on the element report, assert or rule. It is a simple name, not a phrase.</report>
      ...
      <report test="@subject and not(self::sch:report or self::sch:assert)"
		>The attribute subject can only appear on the elements report and assert. It is an XSLT pattern. </report>
      <report test="@test and not(self::sch:assert or self::sch:report)"
		>The attribute test can only appear on the elements report and assert. It is an XPath expression with the XSLT additional functions.</report>
  ...
  <diagnostics>
	...
	>
      ...
      name, value-of, emph and dir.
    </diagnostic>
    <diagnostic id="bad-parent"
	>
      The element appeared inside a <value-of select="name(parent::*)"/>.
    </diagnostic>
  </diagnostics>
</schema>
