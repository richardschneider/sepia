...
xml:lang="en" >
  ...
  <sch:p>
    The Schematron Validation Report Language is a simple language
  ...
  <sch:pattern>
    ...
    <sch:rule abstract="true" id="second-level">
      <!-- was test="../schematron-output" -->
      <sch:assert test="parent::svrl:schematron-output">
    ...
    <sch:rule abstract="true" id="empty">
      ...
      <sch:assert test="string-length(translate(., ' ', '')) = 0">
...
count(svrl:successful-report) = count(*)">
    ...
    <!-- the rule is wrong.  a svrl:diagnostic-reference can have zero or one text elements. -->
    <sch:rule context="svrl:diagnostic-reference">
      ...
      -->
      <sch:assert test="count(svrl:text) = count(*)">
        The <sch:name/> element should only contain a text element.
      </sch:assert>
      <sch:assert test="count(svrl:text) = 1">
        The <sch:name/> element should only contain one text element.
      </sch:assert>
      <sch:assert test="parent::svrl:failed-assert | parent::svrl:successful-report">
        The <sch:name/> element is a child of a failed-assert or a successful-report.
      </sch:assert>
    </sch:rule>
...
or following-sibling::svrl:ns-prefix-in-attribute-value">
    ...
    <sch:rule context="svrl:fired-rule">
      <sch:extends rule="second-level" />
      <sch:extends rule="empty" />
      <!-- was test='preceding-sibling::active-pattern | -->
      <sch:assert
...
preceding-sibling::svrl:successful-report">
  ...
  <!-- A rule can fire multiple times; thus it will be in the SVRL muliple time
         and so will its id.  The following rule cannot be applied.   Also
         the test was missing the closing ')'.
